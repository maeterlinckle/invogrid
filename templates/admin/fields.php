...
$table = static function (array $rows): string {
    ob_start();
    ?>
    <div class="table-wrap">
        <table class="table">
            <caption class="sr-only">Custom fields, their types and the hint each gives the model</caption>
            <thead>
                <tr>
...
<?php if ($produced !== []): ?>
    ...
    <p class="muted">
        Produced by the submission rather than read off the page — the Clear Books id, and the
        number Clear Books assigned — and written onto the document once Clear Books has
        answered. They are <strong>never</strong> offered to the extraction prompt: asking a model
        to find a Clear Books bill id on a supplier's invoice asks it to invent a number that does
